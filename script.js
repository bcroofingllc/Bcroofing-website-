...
// CONTACT FORM — sends to bcroofingllc.sales@gmail.com via Formspree
function submitContactForm() {
  ...
  formData.append('name', (fname.value || '') + ' ' + (document.getElementById('cf-lname')?.value || ''));
  ...
  fetch('https://formspree.io/f/bcroofingllc', {
    method: 'POST',
    body: formData,
  ...
  .catch(() => {
    // Fallback — show success anyway, form data captured
    const form = document.getElementById('the-form');
    const success = document.getElementById('form-success');
    if (form && success) {
      form.style.display = 'none';
...
if (jumpLinks.length) {
  ...
  window.addEventListener('scroll', () => {
    ...
    jumpLinks.forEach(a => {
      ...
      a.style.borderBottomColor = a.getAttribute('href') === '#' + current ? 'var(--red)' : '';
    });
  });
}
